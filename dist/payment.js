...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function disableOrderTypeButtons(disable) {
        if (pickupOption)
            pickupOption.disabled = disable;
        if (deliveryOption)
            deliveryOption.disabled = disable;
    }
    ...
    function observePayPalWindow() {
        const paypalWindow = document.querySelector("iframe[title='PayPal'], iframe.paypal-checkout");
        if (!paypalWindow)
            return;
